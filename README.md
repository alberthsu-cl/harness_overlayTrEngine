...
py -3 harness/src/main.py sync-effect-catalog-sources
...
The deterministic built-in effect catalog lives at [configs/effect_catalog.json](configs/effect_catalog.json). Run `sync-effect-catalog-sources` after an `OverlayTrPlugInFx` registration change: it scans `FxInfo.h` and the Visual Studio project, removes stale registered FX IDs, preserves valid catalog metadata and aliases, adds unknown R&D FX IDs with conservative `metadata_status: discovered` entries, and regenerates both catalog JSON files. Use `index-effects` when only the generated catalog needs rebuilding from [configs/effect_catalog_sources.json](configs/effect_catalog_sources.json). The generated catalog records the source manifest path, manifest version, SHA-256 digest, and registration count. The source manifest is validated for effect source type, required fields, style hints, referenced source documents, and the builtin-versus-generated fallback split. Use `audit-effects` to compare the checked-in source manifest against the built-in baseline and see whether any registrations are missing or extra. The command exits non-zero when the audit is not `ok`, and it can also write a reusable JSON audit report with `--output`.
