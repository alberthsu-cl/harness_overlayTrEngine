...
The deterministic built-in effect catalog lives at [configs/effect_catalog.json](configs/effect_catalog.json). Use `index-effects` to regenerate it from the checked-in source manifest at [configs/effect_catalog_sources.json](configs/effect_catalog_sources.json), which keeps the registration list data-driven. The generated catalog records the source manifest path, manifest version, SHA-256 digest, and registration count. The source manifest is validated for duplicate IDs, effect source type, required fields, style hints, referenced source documents, and the builtin-versus-generated fallback split. Use `audit-effects` to compare the checked-in source manifest against the built-in baseline and see whether any registrations are missing or extra. The command exits non-zero when the audit is not `ok`, and it can also write a reusable JSON audit report with `--output`. You can pass `--source-manifest` to point the indexer or audit command at a different checked-in source list.
